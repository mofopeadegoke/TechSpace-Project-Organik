...
class CouchbaseClient:
    ...
    def search_product(self, item):
        try:
            query = f"SELECT products FROM `{self.bucket_name}`.{self.scope_name}.{self.collection_name}"
            query_result = self._cluster.query(query).execute()
...
def search_products():
    ...
    if products:
        ...
        for product in products:
            product_collections = product.get("ProductsCollections") # 'collection name is ProductsCollection', for dunamis, yours is 'items'
            if product_collections and product_collections.get('name') == search_query:
                filtered_products.append(product_collections)
